...
void draw()
{
    ...
    SDL_Rect wind = {0, 0, SCREEN_WIDTH, SCREEN_HEIGHT};
    SDL_RenderFillRect(gRenderer, &wind );

    srand (time(0));
    int howmany = 50 + (rand() % 150);

    for (int i = 0; i < howmany; i++){
        int x = rand() % SCREEN_WIDTH + 1;
        int y = rand() % SCREEN_WIDTH +1;
        int colour = rand() % 255 + 1;
        int size = 4 + (rand() % 6);
        SDL_SetRenderDrawColor(gRenderer, colour, colour, colour, 0);
        SDL_Rect stars = {x, y, size, size};
